"""
Job Cache for Regional Database API
...
class JobCache:
    """
    Cache for API job IDs to avoid creating duplicate requests.
    ...
    CACHE_FILE = Path(__file__).parent.parent.parent.parent / "data" / "reference" / "job_cache.json"
        ...
        try:
            if cls.CACHE_FILE.exists():
                with open(cls.CACHE_FILE, 'r', encoding='utf-8') as f:
                    cache = json.load(f)
                    logger.debug(f"Loaded job cache with {len(cache.get('jobs', {}))} entries")
                    return cache
        except Exception as e:
            logger.warning(f"Could not load job cache: {e}")
    ...
    def save(cls, cache: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(f"Saved job cache to {cls.CACHE_FILE}")
        except Exception as e:
            logger.error(f"Could not save job cache: {e}")
    ...
    def get_job(cls, table_id: str, year_range: Optional[str] = None) -> Optional[str]:
        ...
        Args:
            ...
            year_range: Optional year range key (e.g., '2011-2024')
        ...
        if job_info:
            ...
            # Don't return jobs that were already successfully completed
            # (data already in database)
            if status == "data_loaded":
                logger.info(f"Job {job_id} already loaded to database, skipping")
                return None
            
            logger.info(f"Found cached job: {job_id} (status: {status})")
            return job_id
    ...
    def save_job(cls, table_id: str, job_id: str, year_range: Optional[str] = None) -> None:
        ...
        cls.save(cache)
        logger.info(f"Cached job {job_id} for {cache_key}")
    ...
    def update_status(cls, table_id: str, status: str, year_range: Optional[str] = None) -> None:
        ...
        if cache_key in cache.get("jobs", {}):
            ...
            cls.save(cache)
            logger.debug(f"Updated job status for {cache_key}: {status}")
    ...
    def clear_job(cls, table_id: str, year_range: Optional[str] = None) -> None:
        """
        Remove a job from cache (e.g., after successful completion or to force refresh).
        ...
        if cache_key in cache.get("jobs", {}):
            ...
            cls.save(cache)
            logger.info(f"Cleared job cache for {cache_key}")
    ...
    @classmethod
    def add_existing_job(cls, table_id: str, job_id: str, year_range: Optional[str] = None) -> None:
        """
        Manually add an existing job ID to cache.
        Useful for adding jobs that were created before caching was implemented.
        
        Args:
            table_id: Table identifier
            job_id: Existing job ID from API
            year_range: Optional year range key
        """
        cache = cls.load()
        cache_key = f"{table_id}_{year_range}" if year_range else table_id
        ...
        cache["jobs"][cache_key] = {
            ...
            "year_range": year_range,
            "created_at": datetime.now().isoformat(),
            "status": "ready",  # Assume it's ready since it exists
            "note": "Manually added existing job"
        }
        cls.save(cache)
        logger.info(f"Added existing job {job_id} for {cache_key}")
