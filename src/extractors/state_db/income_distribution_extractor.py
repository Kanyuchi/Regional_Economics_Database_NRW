"""
Income Distribution Extractor for State Database NRW
Regional Economics Database for NRW

Extracts population data by personal net income brackets and gender
from table 12211-9114i.
...
class IncomeDistributionExtractor(StateDBExtractor):
    """
    Extractor for population by personal net income and gender.

    Table: 12211-9114i
    Coverage: Population at main residence by personal net income
             brackets and gender

    Income brackets (9 categories):
    - Under 150 EUR
    - 150 to under 300 EUR
    - 300 to under 500 EUR
    - 500 to under 700 EUR
    - 700 to under 900 EUR
    - 900 to under 1,100 EUR
    - 1,100 to under 1,300 EUR
    - 1,300 to under 1,500 EUR
    - 1,500 EUR and more
    ...
    TABLE_CODE = "12211-9114i"
    TABLE_NAME = "Population by Personal Net Income and Gender"
    ...
    def extract_year(self, year: int) -> pd.DataFrame:
        """
        Extract income distribution data for a specific year.

        Args:
            year: Year to extract (2005-2019)

        Returns:
            DataFrame with extracted data
        """
        logger.info(f"Extracting income distribution data for year {year}")
    ...
    def _parse_csv_data(self, csv_content: str) -> pd.DataFrame:
        ...
        for line in data_lines:
            ...
            # We expect: Year, Region Code, Region Name, Gender, [9 income bracket columns]
            if len(parts) < 13:
            ...
            # Column mapping: 9 income brackets in columns 4-12
            # Note: Column order to be verified with actual data
            record = {
                'year': year,
                'region_code': region_code,
                'region_name': region_name,
                'gender': gender,
                'income_under_150': self._parse_value(parts[4] if len(parts) > 4 else None),
                'income_150_to_300': self._parse_value(parts[5] if len(parts) > 5 else None),
                'income_300_to_500': self._parse_value(parts[6] if len(parts) > 6 else None),
                'income_500_to_700': self._parse_value(parts[7] if len(parts) > 7 else None),
                'income_700_to_900': self._parse_value(parts[8] if len(parts) > 8 else None),
                'income_900_to_1100': self._parse_value(parts[9] if len(parts) > 9 else None),
                'income_1100_to_1300': self._parse_value(parts[10] if len(parts) > 10 else None),
                'income_1300_to_1500': self._parse_value(parts[11] if len(parts) > 11 else None),
                'income_1500_and_more': self._parse_value(parts[12] if len(parts) > 12 else None)
            }
    ...
    def extract_all_years(self) -> pd.DataFrame:
        """
        Extract data for all available years (2005-2019).

        Returns:
            DataFrame with all years combined
        """
        logger.info(f"Extracting income distribution data for years {self.START_YEAR}-{self.END_YEAR}")
    ...
    def save_raw_data(self, df: pd.DataFrame, output_path: Path = None) -> Path:
        ...
        if output_path is None:
            ...
            output_path = output_dir / f'income_distribution_raw_{self.START_YEAR}_{self.END_YEAR}.csv'
...
def main():
    ...
    extractor = IncomeDistributionExtractor()
