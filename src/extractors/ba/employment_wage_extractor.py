"""
BA Employment and Wage Extractor
Federal Employment Agency (Bundesagentur für Arbeit)

Extracts district-level employment and wage data from BA Excel files.
Available period: 2020-2024 (district-level data not available before 2020)

Data Source: Sheet 8.1 - Demographics and wage brackets
"""
...
class EmploymentWageExtractor(BAExtractor):
    """
    Extractor for BA employment and wage data.

    Processes Sheet 8.1 from BA Excel files which contains:
    - Full-time employees subject to social insurance (core group)
    - Demographics: gender, age, nationality, education, skill level
    - Monthly gross wage brackets: <2k, 2-3k, 3-4k, 4-5k, 5-6k, >6k EUR
    - Median wages
    """
    ...
    # Demographic categories to extract
    # These appear in column 2 and represent different breakdowns
    DEMOGRAPHIC_CATEGORIES = {
        'Insgesamt': 'total',
        'Männer': 'male',
        'Frauen': 'female',
        'unter 25 Jahre': 'age_under_25',
        '25 bis unter 55 Jahre': 'age_25_to_55',
        '55 Jahre und älter': 'age_55_and_over',
        'Deutsche': 'german',
        'Ausländer': 'foreigner',
        'ohne Berufsabschluss': 'no_vocational_degree',
        'anerkannter Berufsabschluss': 'recognized_vocational_degree',
        'akademischer Berufsabschluss': 'academic_degree',
        'Helfer': 'skill_assistant',
        'Fachkraft': 'skill_specialist',
        'Spezialist': 'skill_expert',
        'Experte': 'skill_highly_qualified'
    }
    ...
    def __init__(self, data_dir: Path = None):
        ...
        logger.info(f"BA Employment/Wage extractor initialized for {self.data_dir}")

    def extract_year(self, year: int) -> pd.DataFrame:
        """
        Extract employment and wage data for a single year.
        ...
        Returns:
            DataFrame with columns:
                ...
                - region_code
                - region_name
                - demographic_category (e.g., 'total', 'male', 'female', etc.)
                - demographic_value (e.g., 'total', 'male', 'female')
                - total_employees
                - wage_under_2000
                - wage_2000_to_3000
                - wage_3000_to_4000
        ...
        logger.info(f"Extracting {year} data from {filename}")

        try:
            # Read Sheet 8.1 (skip 10 header rows)
            df = pd.read_excel(
                file_path,
                sheet_name='8.1',
                header=None,
            ...
            logger.info(f"Read sheet 8.1: {len(df)} rows, {len(df.columns)} columns")

            # Filter for NRW regions (codes starting with '05') and Germany ('D')
            df_filtered = self.filter_nrw_regions(df, region_col=0)
            ...
            for idx, row in df_filtered.iterrows():
                ...
                demographic_raw = str(row[2]).strip() if pd.notna(row[2]) else "Insgesamt"

                # Map demographic category
                demographic_value = self.DEMOGRAPHIC_CATEGORIES.get(
                    demographic_raw,
                    demographic_raw  # Keep original if not in mapping
                )

                # Determine demographic type
                demographic_type = self._get_demographic_type(demographic_value)
            ...
            logger.info(f"  Demographic categories: {df_result['demographic_value'].nunique()}")
    ...
    def extract_all_years(self) -> pd.DataFrame:
        """
        Extract employment and wage data for all available years (2020-2024).
    ...
    def save_raw_data(self, df: pd.DataFrame) -> Path:
        ...
        filename = f"employment_wage_raw_{min_year}_{max_year}.csv"
    ...
    def _get_demographic_type(self, value: str) -> str:
        """Determine the demographic type from the value."""
        if value in ['total', 'male', 'female']:
            return 'gender'
        elif value.startswith('age_'):
            return 'age_group'
        elif value in ['german', 'foreigner']:
            return 'nationality'
        elif 'degree' in value or 'vocational' in value:
            return 'education'
        elif value.startswith('skill_'):
            return 'skill_level'
        else:
            return 'other'
