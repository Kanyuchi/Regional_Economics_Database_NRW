"""
BA Occupation Extractor
Federal Employment Agency (Bundesagentur für Arbeit)

Extracts employment and wage data by occupation (KldB 2010 classification).
Period: 2020-2024
Source: Sheet 8.4 - Employment by occupation
"""
...
class OccupationExtractor(BAExtractor):
    """
    Extractor for BA employment and wage data by occupation.

    Processes Sheet 8.4 which contains:
    - Full-time employees by KldB 2010 occupation classification
    - Total employee counts
    - Six wage brackets (under 2k, 2-3k, 3-4k, 4-5k, 5-6k, over 6k EUR)
    - Median wages

    KldB 2010 Occupations (~61 categories):
    ...
    def __init__(self, data_dir: Path = None):
        ...
        logger.info(f"BA Occupation extractor initialized for {self.data_dir}")

    def extract_year(self, year: int) -> pd.DataFrame:
        """
        Extract employment and wage data by occupation for a single year.
        ...
        Returns:
            DataFrame with columns:
                ...
                - region_code
                - region_name
                - occupation_name (KldB 2010 classification)
                - total_employees
                - wage_under_2000
                - wage_2000_to_3000
                - wage_3000_to_4000
        ...
        logger.info(f"Extracting {year} occupation data from {filename}")
        ...
        try:
            ...
            logger.info(f"Read sheet 8.4: {len(df)} rows, {len(df.columns)} columns")
            ...
            if district_start_row is None:
                logger.error("Could not find start of district data in Sheet 8.4")
                return pd.DataFrame()
            ...
            for idx, row in df_filtered.iterrows():
                ...
                occupation_name = str(row[2]).strip() if pd.notna(row[2]) else "Insgesamt"
            ...
            logger.info(f"  Occupations: {df_result['occupation_name'].nunique()}")
    ...
    def extract_all_years(self) -> pd.DataFrame:
        """
        Extract employment and wage data by occupation for all available years (2020-2024).
        ...
        if all_data:
            ...
            logger.info(f"  Occupations: {df_combined['occupation_name'].nunique()}")
            return df_combined
        else:
    ...
    def save_raw_data(self, df: pd.DataFrame) -> Path:
        ...
        filename = f"occupation_raw_{min_year}_{max_year}.csv"
...
def main():
    ...
    print(f"\nUnique occupations: {df_2024['occupation_name'].nunique()}")
    print(f"Sample occupations: {df_2024['occupation_name'].unique()[:10].tolist()}")
