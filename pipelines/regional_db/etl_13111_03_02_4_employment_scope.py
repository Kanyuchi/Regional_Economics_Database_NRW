"""
ETL Pipeline: Table 13111-03-02-4
=================================
Employees subject to social insurance at workplace by employment scope (full-time/part-time)
Reference date: June 30

Source: Regional Database Germany (Regionalstatistik)
Table ID: 13111-03-02-4
...
Usage:
    python pipelines/regional_db/etl_13111_03_02_4_employment_scope.py
...
# Pipeline metadata
PIPELINE_INFO = {
    "table_id": "13111-03-02-4",
    "table_name": "Employees at workplace by employment scope (full-time/part-time)",
    "source": "regional_db",
    "category": "labor_market",
    "available_period": "2008-2024",
    "reference_date": "June 30",
    "geographic_level": "districts",
    "indicator_id": 12,  # emp_scope_workplace (Employment by scope at workplace)
}
...
def run_pipeline(years: list = None) -> bool:
    """
    Run the complete ETL pipeline for table 13111-03-02-4.
    ...
    try:
        ...
        # Note: We'll need to add this method to the EmploymentExtractor
        raw_data = extractor.extract_employees_by_scope(years=years)
        ...
        transformed_data = transformer.transform_scope_employment(
            raw_data,
