"""
ETL Pipeline: Table 13312-01-05-4
=================================
...
Available Period: 2000-2023 (24 years)

Description:
    Extracts employment data by economic sector including:
    - All employed persons (not just those subject to social insurance)
    - Breakdown by economic sectors (WZ 2008)
    - Total, male, female
...
# Pipeline metadata
PIPELINE_INFO = {
    "table_id": "13312-01-05-4",
    "table_name": "Employed persons by economic sectors - Annual average",
    "source": "regional_db",
    "category": "labor_market",
    "available_period": "2000-2023",
    "reference_date": "Annual average",
    "geographic_level": "districts",
    "indicator_id": 19,  # employed_by_sector_annual
}
...
def run_pipeline(years: list = None) -> bool:
    """
    Run the complete ETL pipeline for table 13312-01-05-4.
    
    Args:
        years: Optional list of years to extract. If None, uses 2000-2023 (all 24 years).
    ...
    try:
        ...
        # Use specified years or default (2000-2023 = 24 years)
        if years is None:
            years = list(range(2000, 2024))  # 2000-2023
        
        logger.info(f"Extracting {len(years)} years of data: {years[0]}-{years[-1]}")
        
        raw_data = extractor.extract_employed_by_sector(years=years)
        ...
        transformed_data = transformer.transform_employed_by_sector(
            raw_data,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        help="Specific years to extract (default: all 2000-2023)"
    )
    parser.add_argument(
        "--info",
        action="store_true",
