"""
ETL Pipeline: Table 52111-02-01-4
=================================
Branches by selected economic sectors (WZ 2008) - Year

Source: Regional Database Germany (Regionalstatistik)
Table ID: 52111-02-01-4
Category: Business Economy / Sector Distribution
Available Period: 2006-2023 (18 years)

Description:
    Extracts business establishment statistics categorized by economic sectors
    according to WZ 2008 classification (German industry classification):
...
Usage:
    python pipelines/regional_db/etl_52111_02_01_4_branches_sector.py
    python pipelines/regional_db/etl_52111_02_01_4_branches_sector.py --years 2020 2021 2022 2023
...
# Pipeline metadata
PIPELINE_INFO = {
    "table_id": "52111-02-01-4",
    "table_name": "Branches by economic sectors (WZ 2008)",
    "source": "regional_db",
    "category": "business_economy",
    "subcategory": "sector_distribution",
    "available_period": "2006-2023",
    "reference_date": "31.12",
    "geographic_level": "districts",
    "indicator_id": 23,  # branches_by_sector
}
...
def run_pipeline(years: list = None) -> bool:
    """
    Run the complete ETL pipeline for table 52111-02-01-4.
    
    Args:
        years: Optional list of years to extract. If None, uses 2006-2023 (all 18 years).
    ...
    try:
        ...
        # Use specified years or default (2006-2023 = 18 years)
        if years is None:
            years = list(range(2006, 2024))  # 2006-2023
        
        logger.info(f"Extracting {len(years)} years of data: {years[0]}-{years[-1]}")
        
        raw_data = extractor.extract_branches_by_sector(years=years)
        ...
        transformed_data = transformer.transform_branches_by_sector(
            raw_data,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        help="Specific years to extract (default: all 2006-2023)"
    )
    parser.add_argument(
        "--info",
        action="store_true",
