...
Table: 82711-06i (State Database NRW)
...
from src.extractors.state_db.employee_compensation_extractor import EmployeeCompensationExtractor
from src.transformers.employee_compensation_transformer import EmployeeCompensationTransformer
...
def main(test_mode=False):
    """
    Execute ETL pipeline for employee compensation data.

    Args:
        test_mode: If True, extract only 2018-2022 for testing. If False, extract full 2000-2022.
    """
    ...
    if test_mode:
        start_year = 2019
        end_year = 2023
        logger.info("[TEST MODE] Extracting 2019-2023 only")
    else:
        start_year = 2000
        end_year = 2023
        logger.info("[FULL MODE] Extracting 2000-2023 (24 years)")

    logger.info("="*80)
    logger.info("ETL Pipeline: Employee Compensation by Sector (82711-06i)")
    ...
    try:
        ...
        extractor = EmployeeCompensationExtractor()

        # Extract compensation data (year-by-year due to API limitation)
        logger.info(f"Extracting data for {end_year - start_year + 1} years: {start_year}-{end_year}")
        logger.info("This will take a while...")

        raw_data = extractor.extract_compensation_data(
        ...
        transformer = EmployeeCompensationTransformer()

        # Transform data into database format
        # Note: GDP uses indicators 29-40, so compensation starts from 41
        transformed_data = transformer.transform_compensation_data(
            raw_data,
            indicator_id_base=41,
            years_filter=None
        )
        ...
        with db.get_connection() as conn:
            # Count total compensation records
            ...
            logger.info(f"\nTotal Employee Compensation records: {total_count}")
            ...
            result = conn.execute(text("""
                ...
                WHERE i.indicator_id BETWEEN 41 AND 55
            ...
            result = conn.execute(text("""
                ...
                WHERE f.indicator_id BETWEEN 41 AND 55
            """))
        ...
        if test_mode:
            ...
            logger.info("To extract full data (2000-2022), run:")
            logger.info("  python pipelines/state_db/etl_82711_06i_employee_compensation.py --full")
        else:
            logger.info("\n[SUCCESS] EMPLOYEE COMPENSATION TABLE COMPLETE!")
            logger.info("\nNext steps:")
            logger.info("  1. Verify data with SQL queries")
            logger.info("  2. Create analysis scripts for compensation data")
            logger.info("  3. Proceed with next State DB table")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='ETL Pipeline for Employee Compensation (82711-06i)')
    parser.add_argument('--full', action='store_true',
                       help='Run full extraction (2000-2022). Default is test mode (2018-2022)')
    parser.add_argument('--test', action='store_true',
                       help='Run test mode (2018-2022 only)')
    ...
    if args.full:
        ...
        test_mode = True
        logger.info("[INFO] No mode specified. Running in TEST mode (2018-2022).")
